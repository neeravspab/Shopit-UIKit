//
//  CustomerRagistrationVC.swift
//  Shop It
//
//  Created by niravkumar patel on 3/24/22.
//

import UIKit

class CustomerRagistrationVC: UIViewController {
    ...
    @IBAction func btnRegisterClick(_ sender: UIButton) {
        ...
        if (validateFields())
        {
            let modalInfo = AdduserDetailModal(lname: txtLastName.text!, fname: txtFirstName.text!, email: txtEmail.text!, phone: txtPhone.text!, dob: txtDOB.text!, country: txtCountry.text!, state: txtState.text!, city: txtCity.text!, address: txtStreetApt.text!, password: txtPassword.text!, sex: gender, status: "0", discription: "")
            print(modalInfo)
